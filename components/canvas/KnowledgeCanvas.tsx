...
const CONCEPTS = [
  { pos: [0,    0,    0  ] as [number,number,number], size: 5.0 },
  { pos: [-2.2, 0.8, -0.5] as [number,number,number], size: 3.5 },
  { pos: [ 2.0, 1.0, -0.3] as [number,number,number], size: 3.5 },
  { pos: [-1.8,-1.2,  0.2] as [number,number,number], size: 3.0 },
  { pos: [ 2.2,-0.8,  0.4] as [number,number,number], size: 3.0 },
  { pos: [ 0.2, 2.2, -0.3] as [number,number,number], size: 2.8 },
  { pos: [ 0.3,-2.3,  0.5] as [number,number,number], size: 2.8 },
  { pos: [-3.5, 1.5, -1.0] as [number,number,number], size: 2.0 },
  { pos: [-3.2,-0.5,  0.8] as [number,number,number], size: 2.0 },
  { pos: [ 3.4, 1.2, -0.8] as [number,number,number], size: 2.0 },
  { pos: [ 3.5,-1.0,  0.6] as [number,number,number], size: 2.0 },
  { pos: [-1.0, 3.4, -0.5] as [number,number,number], size: 1.8 },
  { pos: [ 1.2, 3.2,  0.4] as [number,number,number], size: 1.8 },
  { pos: [-1.2,-3.3,  0.3] as [number,number,number], size: 1.8 },
  { pos: [ 1.4,-3.1, -0.6] as [number,number,number], size: 1.8 },
];
...
function GraphNodes() {
  ...
  return (
    <points ref={ref}>
      ...
      <shaderMaterial vertexShader={NODE_VERT} fragmentShader={NODE_FRAG}
...
function DataStreams() {
  ...
  const { positions, speeds, offsets, sizes } = useMemo(() => {
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      const a = CONCEPTS[edge[0]].pos, b = CONCEPTS[edge[1]].pos;
      const t = Math.random();
  ...
  return (
    <points ref={ref}>
      ...
      <shaderMaterial vertexShader={STREAM_VERT} fragmentShader={STREAM_FRAG}
        uniforms={uniforms} transparent depthWrite={false} blending={THREE.AdditiveBlending} />
    </points>
  );
}
...
export default function KnowledgeCanvas({ scrollProgress: _ }: KnowledgeCanvasProps) {
  return (
